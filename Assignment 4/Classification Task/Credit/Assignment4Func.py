...
def log_reg(x_train,y_train,**kwargs):
    ...
    y=np.expand_dims(y,axis=1)
    x=np.concatenate((bias,x_train),axis=1)
    x1=np.concatenate((bias,x_train),axis=1)
    learning_rate=0.001
    W=kwargs.get('W',np.random.uniform(low=-1,high=1,size=(x_train.shape[1]+1)))
    
    for i in range(100):
        learning_rate=1
        prediction=sigmoid(W,x1)#Predict
        prediction[prediction>=0.5]=1
        prediction[prediction<0.5]=0
        error_vec=(y-prediction).T
        error=error_vec[error_vec!=0]
        error=len(error)    
        grad=[sigmoid(W,x1)-y][0]
        grad=np.sum((grad*x1),axis=0)        
        W=W-learning_rate*grad
        if error==0:
            break
    return W
...
def batch_trainer(the_classifier,x_train,y_train): 
    """This function splits the training data and trains the model on it using a fixed batch size """
    batch_size=10
    ...
    for batch in range(len(y_train)):
        if batch==0:
            W=the_classifier(x_train[batch],y_train[batch])
        else:
            W=the_classifier(x_train[batch],y_train[batch],W=W)
    return W
...
def Plot_SubPlots(data,title_name,x_name,y_name):
    ...
    # PERCEPTRON
    percep_pred=batch_trainer(pocket_percep,x_train,y_train) #Get Model Weights
    [error_percep,_]=linear_classify(percep_pred,x_test,y_test) #Get Accuracies
    ...
    # LINEAR LEAST SQUARES
    linear_pred=linear_least_squares(x_train,y_train)# NO batch training here
    # linear_pred=batch_trainer(linear_least_squares,x_train,y_train)    
    ...
    log_pred=batch_trainer(log_reg,x_train,y_train)
    ...
    flda_pred=batch_trainer(FLDA,x_train,y_train)
...
def Eval(data):
    ...
    percep_pred=batch_trainer(pocket_percep,x_train,y_train)   
    [_,y_pred1]=linear_classify(percep_pred,x_test,y_test)
    
    # LINEAR LEAST SQUARES
    linear_pred=linear_least_squares(x_train,y_train)
    # linear_pred=batch_trainer(linear_least_squares,x_train,y_train)   
    [_,y_pred2]=linear_classify(linear_pred,x_test,y_test)
    
    # LOGISTIC REGRESSION
    # log_pred=log_reg(x_train,y_train)
    log_pred=batch_trainer(log_reg,x_train,y_train)
    [_,y_pred3]=linear_classify(log_pred,x_test,y_test)
    
    # FISCHER LINEAR DISCRIMINANT ANALYSIS
    flda_pred=FLDA(x_train,y_train)
    # flda_pred=batch_trainer(FLDA,x_train,y_train)
    [_,y_pred4]=linear_classify(flda_pred,x_test,y_test)
    ...
    return y_pred1,y_pred2,y_pred3,y_pred4,y_test
#%% PLOT PERFORMANCE
